...
$selection="SELECT * FROM user WHERE id='$sessionUserId' ";
 ...
 if($user){
   if(!empty($_POST['title']) && (!empty($_POST['img_url']))&&(!empty($_POST['description']))&&(!empty($_POST['category']))&&(!empty($_POST['content'])) ){
    ...
    $requete="INSERT INTO article(title,image,category,description,content,user_id) VALUES('$title','$img_url','$category','$description','$content','$sessionUserId')";
    $execute=mysqli_query($connexion,$requete);

    if($execute){
        echo "nouvel article ajouter";
    }else{
        echo "echec";
    }
   }
